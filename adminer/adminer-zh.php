...
h($zi["Name"]);}function
fieldName(array$m,$eg=0){$U=$m["full_type"];$ob=$m["comment"];$pb='<span style="white-space:pre-line;display:inline-block;max-height:128px;overflow:hidden">'.h($ob).'</span>';return'<span title="'.h($U.($ob!=""?($U?": ":"").$ob:'')).'">'.h($m["field"]).($ob?'<br />'.$pb:'').'</span>';}function
